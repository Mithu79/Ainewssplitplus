...
import type { ArticleLanguage, CategoryId, FeedSource, GoogleNewsLocale } from "./types";
 ...
 *
 * Categorisation policy (multilingual by design):
 * - Every non-local category carries English mastheads **plus** Bengali (bn)
 *   and Hindi (hi) feeds, so "Top stories" and each category page mix all
 *   three languages. Tamil (ta) feeds also exist but stay opt-in via
 *   `/api/news?lang=ta` and the native-language rail.
 * - The **Local News** desk fetches only popular Bengali publishers
 *   (West Bengal + Bangladesh), all filed in বাংলা.
 */

/** Google News `hl/gl/ceid` triples for the Indian-language feeds. */
const LANGUAGE_LOCALES: Record<Exclude<ArticleLanguage, "en">, GoogleNewsLocale> = {
  bn: { hl: "bn", gl: "IN", ceid: "IN:bn" },
  hi: { hl: "hi", gl: "IN", ceid: "IN:hi" },
  ta: { hl: "ta", gl: "IN", ceid: "IN:ta" },
};

const NATIVE_LANGUAGE_NAME: Record<Exclude<ArticleLanguage, "en">, string> = {
  bn: "বাংলা",
  hi: "हिन्दी",
  ta: "தமிழ்",
};

function googleNews(topic: string, category: CategoryId, id: string, language?: ArticleLanguage): FeedSource {
  const fixedLocale = language && language !== "en" ? LANGUAGE_LOCALES[language] : undefined;
  return {
    id,
    name: language && language !== "en" ? `Google News ${NATIVE_LANGUAGE_NAME[language]}` : "Google News",
    category,
    kind: "google-news",
    ...
    language,
    resolveUrl: ({ locale }) => {
      const l = fixedLocale ?? locale;
      return `https://news.google.com/rss/headlines/section/topic/${topic}?hl=${l.hl}&gl=${l.gl}&ceid=${l.ceid}`;
    },
  };
}

/**
 * Google News `site:` search for one publisher. Several major Bengali and
 * Hindi publishers do not expose a working RSS endpoint (their `/rss` paths
 * return 404 or HTML), so their coverage is routed through Google News the
 * same way Anandabazar and Daily Thanthi always have been.
 */
function publisherViaGoogleNews(options: {
  id: string;
  name: string;
  domain: string;
  category: CategoryId;
  ...
  const { id, name, domain, category, language, weight = 3 } = options;
  const locale = language !== "en" ? LANGUAGE_LOCALES[language] : config.googleNewsLocale;
  return {
    id,
    name,
    category,
    kind: "google-news",
    url: `https://news.google.com/rss/search?q=${encodeURIComponent(`site:${domain} when:2d`)}&hl=${locale.hl}&gl=${locale.gl}&ceid=${locale.ceid}`,
    site: `https://${domain}`,
    weight,
    language,
  };
...
export const FEED_SOURCES: FeedSource[] = [
  // ── World (English mastheads + Bengali & Hindi desks) ───────────────────
  ...
  // Bengali + Hindi general headlines feed the multilingual front page mix.
  ...
  publisherViaGoogleNews({ id: "aajtak", name: "आज तक", domain: "aajtak.in", category: "world", language: "hi", weight: 4 }),
  publisherViaGoogleNews({ id: "jagran", name: "दैनिक जागरण", domain: "jagran.com", category: "world", language: "hi", weight: 4 }),
  // Tamil stays registered but opt-in (lang=ta / the language rail).
  ...
  publisherViaGoogleNews({ id: "dailythanthi", name: "தினத்தந்தி", domain: "dailythanthi.com", category: "world", language: "ta" }),
  ...
  googleNews("TECHNOLOGY", "tech", "gn-tech-bn", "bn"),
  googleNews("TECHNOLOGY", "tech", "gn-tech-hi", "hi"),
  ...
  googleNews("BUSINESS", "business", "gn-business-bn", "bn"),
  googleNews("BUSINESS", "business", "gn-business-hi", "hi"),
  ...
  googleNews("SPORTS", "sports", "gn-sports-bn", "bn"),
  googleNews("SPORTS", "sports", "gn-sports-hi", "hi"),
  ...
  googleNews("SCIENCE", "science", "gn-science-bn", "bn"),
  googleNews("SCIENCE", "science", "gn-science-hi", "hi"),
  ...
  googleNews("HEALTH", "health", "gn-health-bn", "bn"),
  googleNews("HEALTH", "health", "gn-health-hi", "hi"),

  // ── Entertainment (en + bn + hi) ────────────────────────────────────────
  ...
  googleNews("ENTERTAINMENT", "entertainment", "gn-entertainment-bn", "bn"),
  googleNews("ENTERTAINMENT", "entertainment", "gn-entertainment-hi", "hi"),

  // ── Local News: popular Bengali publishers only ──────────────────────────
  // The Local desk is deliberately narrow — it fetches exclusively from the
  // biggest Bengali newsrooms (West Bengal + Bangladesh), all filed in বাংলা.
  // Direct `/rss` endpoints for these publishers return 404 or HTML, so each
  // one is routed through a Google News `site:` query (the pattern proven by
  // Anandabazar and Daily Thanthi). Extra operator-supplied feeds can still be
  // merged in through the LOCAL_FEEDS env var (see `customLocalSources`).
  publisherViaGoogleNews({ id: "anandabazar", name: "আনন্দবাজার পত্রিকা", domain: "anandabazar.com", category: "local", language: "bn", weight: 4 }),
  publisherViaGoogleNews({ id: "prothomalo", name: "প্রথম আলো", domain: "prothomalo.com", category: "local", language: "bn", weight: 4 }),
  publisherViaGoogleNews({ id: "eisamay", name: "এই সময়", domain: "eisamay.com", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "sangbadpratidin", name: "সংবাদ প্রতিদিন", domain: "sangbadpratidin.in", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "bartaman", name: "বর্তমান", domain: "bartamanpatrika.com", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "aajkaal", name: "আজকাল", domain: "aajkaal.in", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "ebela", name: "এবেলা", domain: "ebela.in", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "yugantor", name: "দৈনিক যুগান্তর", domain: "yugantor.com", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "samakal", name: "সমকাল", domain: "samakal.com", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "kalerkantho", name: "কালের কণ্ঠ", domain: "kalerkantho.com.bd", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "abp-ananda", name: "ABP আনন্দ", domain: "abpanda.com", category: "local", language: "bn" }),
  publisherViaGoogleNews({ id: "zee24ghanta", name: "জি২৪ ঘণ্টা", domain: "zee24ghanta.com", category: "local", language: "bn" }),
];
